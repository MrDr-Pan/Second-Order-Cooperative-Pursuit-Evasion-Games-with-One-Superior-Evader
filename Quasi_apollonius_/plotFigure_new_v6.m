...
x_filtered=x_filtered(589:end);
y_filtered=y_filtered(589:end);
disp('筛选后的 x 坐标:');
disp(x_filtered);
disp('筛选后的 y 坐标:');
disp(y_filtered);
...
for i = 2:n
    % 计算当前点到未访问点的距离
    dist = sqrt((x_filtered(unvisited) - x_filtered(current_idx)).^2 + (y_filtered(unvisited) - y_filtered(current_idx)).^2);
...
para=1.5;
% 绘制每个闭合图形
plot(-x_filtered(order), y_filtered(order), 'Color', 'k', 'LineWidth', 6*para, ...
    'LineStyle', '-');
plot(-x_filtered(order), y_filtered(order), 'Color',   [0.0, 176, 80]/255, 'LineWidth', 4*para, ...
    'LineStyle', '-');
plot(-x_filtered(order), y_filtered(order), 'Color', 'w', 'LineWidth', 2*para, 'LineStyle', ':');
...
% y_filtered=y(1:228);
% 
x_filtered=x(229:588);
y_filtered=y(229:588);
...
x1 = -0.520024;   % 第一个点的x坐标
y1 = 0.145105;   % 第一个点的y坐标
x2 = -0.537977;    % 第二个点的x坐标
y2 = 0.0526523; % 第二个点的y坐标

% % 设置插值的x值
x_interp = linspace(x1, x2, 100);  % 生成100个从x1到x2之间的点

% 使用线性插值
y_interp = interp1([x1, x2], [y1, y2], x_interp, 'linear');

x_filtered=[x_filtered;x_interp'];
y_filtered=[y_filtered;y_interp'];


x1 = -0.537977;   % 第一个点的x坐标
y1 = -0.0526523;   % 第一个点的y坐标
x2 = -0.537978;    % 第二个点的x坐标
y2 = 0.0526523; % 第二个点的y坐标
...
x1 = -0.537977;   % 第一个点的x坐标
y1 = -0.0526523;   % 第一个点的y坐标
x2 = -0.520024;   % 第二个点的x坐标
y2 = -0.145105;  % 第二个点的y坐标
...
%---------------------
x1 = -0.466059;   % 第一个点的x坐标
y1 = 0.0505505;   % 第一个点的y坐标
x2 = -0.466058;   % 第二个点的x坐标
y2 = -0.0505505;  % 第二个点的y坐标
