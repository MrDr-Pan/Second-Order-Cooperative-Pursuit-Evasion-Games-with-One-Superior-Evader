...
class MAPPOAgent:
    def __init__(
        ...
        if continuous:
            self.actor = GaussianActor(obs_dim, act_dim, hidden_dims=actor_hidden_dims).to(self.device)
        else:
            self.actor = CategoricalActor(obs_dim, act_dim, hidden_dims=actor_hidden_dims).to(self.device)
        self.critic = Critic(self.state_dim, hidden_dims=critic_hidden_dims, output_dim=value_output_dim).to(self.device)
