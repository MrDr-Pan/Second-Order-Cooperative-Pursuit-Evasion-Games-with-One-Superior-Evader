...
class MADDPGAgent:
    def __init__(
        ...
        self.actor = DeterministicActor(obs_dim, act_dim, hidden_dims=actor_hidden_dims, act_limit=act_limit).to(device)
        self.critic = Critic(n_agents * (obs_dim + act_dim), hidden_dims=critic_hidden_dims, output_dim=critic_output_dim).to(device)
