...
2. How multiple pursuers should coordinate to maintain encirclement integrity and eventually capture a superior evader. :contentReference[oaicite:4]{index=4}
...
- A **decoder-only Transformer prediction network** is incorporated to predict the evader’s future motion from historical states, helping pursuers make more proactive decisions. :contentReference[oaicite:5]{index=5} :contentReference[oaicite:6]{index=6}
...
3. **A predictive multi-agent reinforcement learning framework**  
   A MADRL-based encirclement strategy is developed, enhanced with a decoder-only Transformer predictor that estimates the evader’s future trajectory and improves cooperative pursuit performance. :contentReference[oaicite:7]{index=7}
...
- pursuers do not know the exact evasion strategy in advance, but can estimate it from historical information. :contentReference[oaicite:8]{index=8}
...
This makes QAC a more suitable geometric object for analyzing pursuit-evasion interactions in second-order systems. :contentReference[oaicite:9]{index=9}
...
Encirclement is considered feasible when the occupied angles induced by all pursuers jointly cover the full angular domain around the evader without gaps. :contentReference[oaicite:10]{index=10} :contentReference[oaicite:11]{index=11}
